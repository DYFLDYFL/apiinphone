...
主能力：DeepSeek 官方 API、工具调用、多会话、用量面板；另有全屏 **多智能体游戏模式**。

当前版本：**1.5.1** / versionCode **51**。以 `package.json` 与 `android/app/build.gradle` 为准。

## 构建（改完代码必打）

仓库规则：改动 App 行为后必须执行 `.\build.ps1`（见 `.cursor/rules/auto-build-apk.mdc`）。  
默认 patch 升版本；手动指定版本时使用 `-SkipVersionBump`。
...
- `settingsForGame()` 使用官方 API 的游戏模型配置。
- 创建/推进只校验 `apiKey`；页脚不再显示传输通道。
- `ModelSwitcher` `scope="game"` 使用独立游戏模型和推理档位。
...
- 默认开场：`三月初二 05:30`（`DEFAULT_INITIAL_TIME_PARTS`）
...
- 官方 API 软限流可自动重试
- 建局模板可配统一 AI 列表、提示词、模型、信息差权限和串行/并行流水线
